...
async function displayJSONData(memos) {
    ...
    memos.forEach(memo => {
        ...
        const dateStrlong = date.toLocaleString("en-us", { weekday: "long", year: "numeric", month: "long", day: "numeric" });
        ...
        cardLink.setAttribute("style", "TEXT-DECORATION: none;color:#555; font-size:10px; font-family:monospace;");
        cardLink.href = memosUrl + '/m/' + memo.uid; // 设置链接的目标 URL
        cardLink.setAttribute('target', '_blank');
        cardLink.textContent = memosUrl + '/m/' + memo.uid; // 设置链接的文本内容
        ...
        cardaNamelink.name = dateStr; // 设置锚点
        ...
        if (memo.resources && memo.resources.length > 0) {
            ...
            memo.resources.forEach(resource => {
                ...
                if (/\.(jpeg|jpg|gif|png|bmp|webp|svg)/i.test(resource.filename)) {
